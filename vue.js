...
sharePage() {
  ...
  // Only put descriptive text here
  const shareText = `${platformName}/${brandName} - Check out this store!`;
  
  if (navigator.share) {
    navigator.share({
        title: `${platformName} Store`,
        text: shareText,
        url: pageURL // <-- the URL goes in the `url` field, not inside text
      })
      .then(() => this.showToast("Shared successfully!", "success"))
      .catch(err => this.showToast("Share failed", "error"));
  } else {
